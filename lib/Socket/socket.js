...
const makeSocket = (config) => {
   ...
   const SAFE_TIMEOUT = connectTimeoutMs + 2000;
   ...
   const tryReconnect = () => {
      ...
      reconnectAttempts++;
      logger.warn(`Reconnecting... Attempt ${reconnectAttempts}`);
      setTimeout(() => {
         (0, exports.makeSocket)(config); // rekursif: panggil ulang makeSocket
   ...
   const end = (error) => {
      ...
      if (error?.output?.statusCode !== Types_1.DisconnectReason.loggedOut) {
         tryReconnect();
      }
   ...
   const sendRawMessage = async (data) => {
      ...
      await (0, Utils_1.promiseTimeout)(SAFE_TIMEOUT, async (resolve, reject) => {
         try {
   ...
   const logout = async (reason) => {
      ...
      end(new boom_1.Boom(reason || 'Intentional Logout', {
   ...
   const awaitNextMessage = async (sendMsg) => {
      ...
      let onOpen;
      let onClose;
      const result = (0, Utils_1.promiseTimeout)(SAFE_TIMEOUT, (resolve, reject) => {
         onOpen = resolve;
   ...
   const requestPairingCode = async (phoneNumber, pairKey = "RENNBAIL") => {
      if (pairKey) {
